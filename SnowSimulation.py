...
Code Reflection: A lot of work was put into this project. Originally we thought of doing a much simpiler simulation just on a small 3d object, but there was an idea of scaling it up to Choate and the area around choate. We first had to find a height map of wallingford which turned out to be one of the most challenging aspects of the simulation. Originally the heightmap included unnecessary overlays of roads, buildings, and text, which messed up the simulation. We wrote a program to remove all of the unnecessary things. The next problem we ran into was with how the height map represented its values, a gradient from blue to green to yellow to red. This is very clear to the human eye but a lot harder for a computer to understand. Our first thought was to imagine the colors in 3d space and take the distance between the color coordinate and the lowest possible color. (R, G, B) would correspond to (X, Y, Z). However the green was a lot closer to the red which gave us a height map where the blue was really low and the green and red were about the same height and really high. Our next idea was to convert the image to CMY(see HeightGen.py for explanation), which was the idea we ended up sticking with. The simulation itself was coded off of many factors and research. We researched the terminal velocity of snowflakes, wind patterns and movement, rate of change of wind's direction, and the general fall speed of snow. The simulation code came together generally well with little issues, the harderst and most tedious part was extracting the data from the height map. There are many things that we can improve on. Around the edges of the final image we see that there is very little snow. This is because the wind carries the snow either away from the edges or it carries it all the way to the edge of the image; when the snowflake hits the edge it's respawned randomly for simplicity. In order to increase the amount of snow fall on the edges we could increase the area of snowfall, extending it beyond the image around 500 feet. We repeated this simulation many times to see whether we were getting consistent results.

Simulation Reflection: This simulation was overall accurate. In the simulation, we found that the average amount of snowfall across Wallingford in one hour was 0.5 inches. Looking at real-world data, 0.5 inches is a small amount of snowfall for one hour; however, in all of Wallingford, we are only creating 150 snowflakes per pixel, which is 150 snowflakes per square meter on the map. We simulated fewer snowflakes than there would actually be falling in Wallingford. Therefore, it is reasonable that we got 0.5 inches in our simulation, which corresponds to light snowing in the real-world. In our simulation, there were no significant outliers. There were a couple of chunks that had higher chunk number than others. One chunk had 240, another had 232, and one had 235, which is all much higher than the average of 150. These higher chunk numbers were all on the top right, where the terrain was the highest, so it isn't unusual that some chunks got these high numbers. In this project, we brought variable bias. We couldn't have possibly incorporated all of the variables. We did, however, get many of the variables and modeled them accurately. We used average data for wind, and how wind changes in direction and magnitude over time. The wind is one of the biggest variables accouting snowfall. We think that if we had more time, it would be interesting to model the temperature of the ground. Based on the starting temperature and ambient air temperature, how would that affect the height of snowfall. If the ground is warm, the snowfall will melt and not stick to the surfacee, but as snow falls and hits the ground, it will start to make the ground cooler, and the snow will start to stick. We aproached biases very carefully. We checked many sources while being aware of confirmation bias. I think that it would be interesting to see this simulation with modeling ground temperature, because it may change the variation between low and high ground comparatively to our current simulation. From this simulation, we learned how much height differences vary the amount of snowfall. In the grand scheme of the simulation, there was not that much difference between the low and high areas, but we found it particularly interesting how prominent the ridges were across the final image. It practically looks like a contour map of Wallingford. We are not 100% sure why this happens, but it is interesting to see.
